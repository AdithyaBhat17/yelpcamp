...
const AddCampground = ({ history, location }: RouteComponentProps) => {
  const [data, loading] = useFetch(
    `${process.env.REACT_APP_BASE_URL}/authorized/`
  );
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    const { name, file, description, price } = state;
        ...
        <div className='col-lg-6 col-sm-12'>
          ...
          <form encType='multipart/form-data' onSubmit={handleSubmit}>
            <div className='form-group'>
              <label className='campground-label' htmlFor='name'>
            ...
            <div className='form-group'>
              <label className='campground-label' htmlFor='description'>
                Describe {state.name || 'Campground'}
              </label>
              <textarea
                className='form-control campground-input'
                name='description'
            ...
            <div className='form-group'>
              <label className='campground-label' htmlFor='image'>
                Campground image
              </label>
              <input
                className='form-control campground-input'
                ...
                onFocus={() => error.length && setError('')}
                onChange={e =>
                  e.target.files !== null && toBase64(e.target.files[0])
                }
              />
            </div>
            <div className='form-group'>
              <label className='campground-label' htmlFor='price'>
                Price
              </label>
              <input
                type='number'
            ...
            <input
              className='auth-btn camp-btn'
              type='submit'
              value={`${submit ? 'Adding' : 'Add'} Campground`}
            />
          </form>
        </div>
        <div className='col-lg-6 col-sm-12'>
