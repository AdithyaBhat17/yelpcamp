...
const Campgrounds: React.FC<Props> = ({ search }) => {
  const [data, loading] = useFetch(
    `${process.env.REACT_APP_BASE_URL}/campgrounds`
  );
  ...
  const renderCampgrounds = (
    ...
    for (let camp = 0; camp < campgrounds.length; camp++) {
      if (campgrounds[camp].name.toLowerCase().includes(search))
        campgroundsJSX.push(
          <div
            key={campgrounds[camp]._id}
            className='col-lg-3 col-md-6 col-sm-12'
          >
            <div className='thumbnail campground-cards'>
              <img
                className='thumbnail-img'
                src={campgrounds[camp].image}
                alt={campgrounds[camp].name}
              />
    ...
    if (campgroundsJSX.length) return campgroundsJSX;
    else
      return (
        <div className='empty'>
